...
from vipe.bench.replica import ReplicaDataset
from vipe.bench.replica_instance import evaluate_scene
...
WORKER_ENV = "_VIPE_REPLICA_INSTANCE_WORKER"
SCENE_QUEUE_ENV = "_VIPE_REPLICA_INSTANCE_SCENE_QUEUE"
PIPELINE_CONFIG_PATH = get_config_path() / "default.yaml"
INSTANCE_CONFIG_PATH = get_config_path() / "default_instance.yaml"
EVAL_CONFIG_PATH = get_config_path() / "eval_replica_instance_config.yaml"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build and evaluate ViPE Replica instance hypotheses.")
    parser.add_argument("--scenes", nargs="*", default=None, help="Replica scenes; defaults to extracted scenes")
    ...
    parser.add_argument("--raw-root", required=True, type=Path, help="Replica_full root with semantic meshes")
...
def _manifest_path(args: argparse.Namespace, scene: str) -> Path:
    return args.work_dir / "model_results" / "replica" / scene / "instance" / "manifest.json"
...
def _request_fingerprint(args: argparse.Namespace, scene: str) -> dict[str, str | int]:
    ...
    return {
        ...
        "pipeline_config_sha256": _sha256_files([PIPELINE_CONFIG_PATH, INSTANCE_CONFIG_PATH]),
    }


def _scene_complete(args: argparse.Namespace, scene: str) -> bool:
    manifest_path = _manifest_path(args, scene)
    ...
    try:
        ...
        return False
    if manifest.get("request") != _request_fingerprint(args, scene):
        return False
    return all(path.is_file() for path in _instance_paths(_output_dir(args, scene), scene))


def _write_manifest(args: argparse.Namespace, scene: str) -> None:
    paths = _instance_paths(_output_dir(args, scene), scene)
    missing = [str(path) for path in paths if not path.is_file()]
    if missing:
        raise FileNotFoundError("Missing ViPE instance artifacts:\n" + "\n".join(missing))
    _write_json(
        _manifest_path(args, scene),
        {
            "format": "vipe_replica_instance_manifest_v1",
            "scene": scene,
            "request": _request_fingerprint(args, scene),
            "artifacts": {
                name: str(path.resolve())
                for name, path in zip(("pose", "tsdf", "instances", "instance_summary", "instance_ply"), paths)
            },
        },
    )


def run_vipe(scene_dir: Path, output_dir: Path, pipeline_cfg, instance_cfg) -> None:
...
def _run_scene_subprocess(args: argparse.Namespace, scene: str) -> tuple[float, float]:
...
def _write_queue(path: Path, scenes: list[str]) -> None:
...
def _evaluate(args: argparse.Namespace, scene: str, eval_cfg) -> dict:
    cache_dir = args.work_dir / "model_results" / "replica" / scene / "instance" / "eval_cache"
...
def _run_worker(args: argparse.Namespace, eval_cfg) -> dict:
    payload = {"build": {}, "metric_eval": {}, "metrics": {}, "failures": {}}
    for scene in _worker_scenes(args):
        try:
            frames = scene_frame_count(_scene_dir(args, scene))
            if not _scene_complete(args, scene):
                shutil.rmtree(_output_dir(args, scene), ignore_errors=True)
                shutil.rmtree(_manifest_path(args, scene).parent, ignore_errors=True)
                seconds, peak_vram = _run_scene_subprocess(args, scene)
                _write_manifest(args, scene)
            ...
            if args.do_final_eval:
                start = time.perf_counter()
                payload["metrics"][scene] = _evaluate(args, scene, eval_cfg)
            ...
            print(f"[WARN] Skipping failed Replica instance scene | {scene} | {exc}", flush=True)
        finally:
...
def _spawn_workers(args: argparse.Namespace) -> list[dict] | None:
    ...
    for worker_id, gpu in enumerate(gpus):
        ...
        print(f"[INFO] Starting Replica instance worker {worker_id} on GPU {gpu}", flush=True)
        process = subprocess.Popen(
            base + ["--gpu-id", str(worker_id), "--total-gpus", str(len(gpus))],
            env=env,
        )
    ...
    if failed_workers:
        print(f"[WARN] Replica instance worker failures: {failed_workers}", flush=True)
    payloads = []
...
def _write_results(args: argparse.Namespace, payload: dict, eval_cfg) -> dict:
    ...
    if payload["failures"]:
        _write_json(_metric_dir(args) / "replica_instance_failed_scenes.json", payload["failures"])
...
def _print_summary(result: dict) -> None:
    ...
    print("\n" + "=" * 54)
    print("REPLICA INSTANCE DISTILLATION SUMMARY")
    print("=" * 54)
...
def main() -> None:
    ...
    instance_cfg = load_yaml_config(INSTANCE_CONFIG_PATH)
    eval_cfg = load_yaml_config(EVAL_CONFIG_PATH)
    ...
    if args.scene_worker_scene is not None:
        ...
        run_vipe(_scene_dir(args, args.scene_worker_scene), args.scene_worker_output_dir, pipeline_cfg, instance_cfg)
        return

    dataset = ReplicaDataset(args.input_root, args.raw_root, eval_cfg)
    ...
    if os.environ.get(WORKER_ENV) != "1":
        (_metric_dir(args) / "replica_instance_failed_scenes.json").unlink(missing_ok=True)
        ...
        if skipped:
            _write_json(
                _metric_dir(args) / "replica_instance_skipped_gt_pose_scenes.json",
                {scene: report.summary() for scene, report in skipped},
            )

    spawned = _spawn_workers(args)
    if spawned is None:
        payload = _run_worker(args, eval_cfg)
        if os.environ.get(WORKER_ENV) == "1":
            return
    else:
        payload = _merge_payloads(spawned)
    result = _write_results(args, payload, eval_cfg)
    _print_summary(result)
