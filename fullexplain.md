...
Stage 6 receives the compact native TSDF point/normal pairs selected during Stage 5 and chooses motion-spaced views. Stage 7 builds normal-aware surface atoms. Stage 8 generates and propagates SAM masks, projects the compact surface into each retained frame, directly accumulates sparse atom-mask and atom-frame count tables plus fixed-size adjacency affinity statistics, and discards each normalized frame signature. Stages 9-11 build the hierarchy, form evidence candidates, and select the final overlapping `K=5` hypotheses. Their full computation and artifacts are specified in [`ALGORITHM.md`](ALGORITHM.md). Ground-truth labels and dataset-specific exclusions are benchmark-only and never enter these runtime stages.
